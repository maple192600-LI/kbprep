...
describe("kbprep worker pipeline - output guards part 1", () => {
  it("copies local Markdown image assets into run and latest outputs", () => {
    ...
    try {
      ...
      expect(existsSync(result.data.outputs.clean_view)).toBe(true);
    ...
    try {
      runPython(
        [
          ...
          "assert payload['error']['details']['legacy_code'] == 'E_QA_FAILED', payload",
    ...
    try {
      runPython(
        [
          ...
          "payload = json.loads(stdout.getvalue())",
          "assert payload['ok'] is False, payload",
          "assert payload['error']['code'] == 'E_SOURCE_CONVERSION_LOSS', payload",
          "assert payload['error']['details']['legacy_code'] == 'E_QA_FAILED', payload",
          "run_dir = Path(payload['error']['details']['run_dir'])",
  ...
  it("restores prepare output when cleaning deletes protected source body content", () => {
    ...
    try {
      runPython(
        [
          "import io, json, sys",
          "from pathlib import Path",
          "from kbprep_worker import clean_rules, classify_blocks, prepare",
          "input_path = Path(sys.argv[1])",
          "output_root = Path(sys.argv[2])",
          "input_path.write_text('# Source Title\\n\\nStep 1: keep threshold=0.8 and verify the account id.\\nCompanion video included for updates.\\n', encoding='utf-8')",
          "original_classify = classify_blocks.classify_blocks",
          "def mark_source_block_protected(blocks, **kwargs):",
          "    classified = original_classify(blocks, **kwargs)",
          ...
          "            block['text'] = 'Step 1: keep threshold=0.8 and verify the account id.'",
          "            block['status'] = 'keep'",
          "            block['risk_tags'] = ['promo_line_removed']",
          "            block['reason'] = 'simulated_unsafe_cleanup'",
          "            promo = dict(block)",
          "            promo['block_id'] = block['block_id'] + '_promo_001'",
          "            promo['text'] = 'Companion video included for updates.'",
          "            promo['type'] = 'marketing_cta'",
          "            promo['protected'] = False",
          "            promo['status'] = 'discard'",
          "            promo['cleaning_rule_id'] = 'base.promotional_line.companion_video'",
          "            promo['cleaning_rule_source'] = 'rules/base/obvious_noise.json'",
          "            promo['reason'] = 'simulated unsafe promo split from protected block'",
          "            derived.append(promo)",
          "            assert original_text != block['text']",
          "    return blocks + derived",
          "classify_blocks.classify_blocks = mark_source_block_protected",
          ...
          "rejected_records = [json.loads(line) for line in rejected_text.splitlines() if line.strip()]",
          "assert len(rejected_records) == 2, rejected_records",
          "assert all(record['schema'] == 'kbprep.rejected_cleaning_patch.v1' for record in rejected_records), rejected_records",
          "assert all(record['reason_code'] == 'protected_structure_change' for record in rejected_records), rejected_records",
          "assert 'threshold=0.8' not in rejected_text, rejected_text",
          "assert 'Companion video included' not in rejected_text, rejected_text",
          "discarded = (run_dir / 'discarded.md').read_text(encoding='utf-8')",
          "assert 'threshold=0.8' not in discarded, discarded",
          "assert 'Companion video included' not in discarded, discarded",
          "cleaned = (run_dir / 'cleaned.md').read_text(encoding='utf-8')",
          "assert 'threshold=0.8' in cleaned, cleaned",
          "assert 'Companion video included' in cleaned, cleaned",
          "assert payload['data']['latest_outputs']['obsidian_dir'] is not None, payload",
