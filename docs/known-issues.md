...
- Image OCR, media ASR, and YouTube routes are verified CLI capabilities with real fixtures; optional-route depth (playlist real-network evidence, YouTube cue-id precision) remains future work.
- Batch manifests now report evidence-backed rerun scope and executable failed/pending rerun. YouTube playlist input now expands into bounded child jobs with per-video parent status, and playlist rerun preserves playlist source-collection evidence. Worker-level `policy_affected` rerun now filters children by run-evidence identity (canonical_ir document_id, cleaning_policy_snapshot_hash, stable source_identity fields); Canonical IR id-level (node-id) narrowing remains tied to later work; the `policy_affected` rerun scope and its `affected_*` identity fields are now exposed through the CLI.
