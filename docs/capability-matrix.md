...
| youtube_url_routes | YouTube URLs | unsupported | design_only | target: subtitle order, transcript text, source URL evidence | none | URL processing depends on network behavior, subtitle availability, external dependencies, timeout handling, and transcript quality; it stays target-only until the technical route contract and fixtures prove it. |
...
1. Add real image OCR, legacy Office, and media ASR fixtures before promoting experimental routes.
