...
`canonical_ir_binding.status` is `bound` when the selected run has `canonical_ir/manifest.json` and `document_manifest.json`; this is run-level evidence. Older or incomplete runs can still report `pending`. The current public feedback rerun selectors are run directory, accepted proposal, and document type promotion history. Pass `--node-ids n_000001,n_000003` to `--plan-rerun`/`--execute-rerun` to narrow worker cleaning to specific canonical node ids (`id_level_narrowing=true` in the binding). Batch and playlist selective rerun are handled by `kbprep-batch --rerun` from `batch_manifest.json`; playlist rerun preserves playlist source-collection evidence in `batch_rerun_manifest.json`.
