...
`canonical_ir_binding.status` is `bound` when the selected run has `canonical_ir/manifest.json` and `document_manifest.json`; this is run-level evidence, not node-id targeting. Older or incomplete runs can still report `pending`. The current public feedback rerun selectors are run directory, accepted proposal, and document type promotion history. Batch and playlist selective rerun are handled by `kbprep-batch --rerun` from `batch_manifest.json`; playlist rerun preserves playlist source-collection evidence in `batch_rerun_manifest.json`. Canonical IR id-level targeting remains outside the shipped claim.
